...
def hostel_detail_view(request, hostel_name):
    try:
        ...
        # Find the one whose slugified name matches the slug in the URL
        for hostel in hostels:
            if slugify(hostel.name) == hostel_name:
                # Found the matching hostel
                context = {
                    'hostel': hostel,
                    'rooms': Room.objects.filter(hostel=hostel).order_by('name')
                }
